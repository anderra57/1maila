...
public class Bezeroa {
	...
	public double saldoaLortu(String pPasaHitza) {
		...
		if(this.pasaHitza==pPasaHitza){
			System.out.println(this.izenAbizena+", zure saldoa " +this.saldoa+ " da");
			saldoa=this.saldoa;
		}
		else{
	...
	public void saldoaEguneratu(String pPasaHitza, double pDiruKop) {
		...
		// adierazteko. Ondo burutu bada,zure saldo berria xx,xx da
		// pantailaratuz, eta gaizki burutu bada, saldoa ezin izan da aldatu
		// inprimatuko luke.
		// double dirua = pDiruKop;

		if (this.pasaHitzaBaieztatu(pPasaHitza) && (this.saldoaLortu(pPasaHitza)>pDiruKop)){
			this.saldoa=this.saldoaLortu(pPasaHitza)-pDiruKop;
		}
		else{
			System.out.println("Saldoa ezin izan da aldatu. Saiatu berriro.");
		}
